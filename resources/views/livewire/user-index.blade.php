              ...
              <tbody>
                @foreach($datas as $no => $data)
                  <tr>
                    ...
                    <td class="th-font" data-title="Email">
                      {{ $data->email }}</td>
                    <td class="th-font" data-title="Aksi">
                      <form action="{{ route('users.destroy', $data->id) }}" method="post">
                        <a class="btn btn-sm btn-primary" href="{{ route('users.show',$data->id) }}" title="Lihat Detail">
                          <i class="fa fa-eye"></i>
                        </a>
                        @if($auth == $data->id)
                          <a class="btn btn-sm btn-warning"
                            href="{{ route('users.edit',$data->id) }}" title="Ubah Data">
